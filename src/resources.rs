...
#[derive(Clone, Debug)]
pub struct ResourcePool(Arc<RwLock<Vec<Resource>>>);

impl ResourcePool {
    pub async fn read(&self) -> RwLockReadGuard<'_, Vec<Resource>> {
        self.0.read().await
    }
    pub async fn write(&self) -> RwLockWriteGuard<'_, Vec<Resource>> {
        self.0.write().await
    }
...
pub struct Resource {
    ...
    pub price_min: f32,
    pub price_max: f32,
    // variation depuis le dernier point de l'historique
    pub var: f32,
    pub historic: Vec<f32>,
    pub coef_volatility: f32,
    pub coef_stength: f32,
}
